...
class NumberGame:
    def __init__(self, root):
        ...
        self.var_start = tk.StringVar(value="Player")  # Default to Human starting
    ...
    def evaluate_state(self, num_string, player_score, computer_score, is_maximizing):
        ...
        # Favor states where the computer has a higher score (maximizing) or player has a lower score (minimizing)
    ...
    def select_number(self, index):
        ...
        # Debug: Print the click event to verify it’s being detected
        ...
        if not hasattr(self, 'selected_index'):
            ...
            print(f"First number selected at index {index}")
        else:
            # Second number selection: check if it's adjacent to the first
            print(f"Second number selected at index {index}, first was {self.selected_index}")
