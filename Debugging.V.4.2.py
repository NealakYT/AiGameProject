...
class NumberGame:
    ...
    def select_number(self, index):
        print(f"Button clicked: index={index}, current_player={self.current_player}")
        ...
        if not hasattr(self, 'selected_index'):
            ...
            print(f"First selection: {index}")
        else:
            if abs(index - self.selected_index) != 1:
                print(f"Selected {self.selected_index} and {index} - not adjacent!")
                ...
                return
            print(f"Processing move: {self.selected_index} and {index}")
